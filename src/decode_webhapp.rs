...
pub async fn save_happ_or_webhapp(
  ...
  let file = std::fs::File::open(happ_or_web_happ_path)?;
  let app_bundle = match WebAppBundle::unpack(file) {
    Ok(web_app_bundle) => {
  ...
  let app_bundle_bytes = app_bundle
    .pack()
    .map_err(|e| napi::Error::from_reason(format!("Failed to write pach app bundle: {}", e)))?;

  fs::write(happ_path, app_bundle_bytes)
